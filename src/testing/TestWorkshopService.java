...
public class TestWorkshopService {
	...
	private static Workshop workshop = new Workshop("testName" ,"testAddress" , "testContact" , "testUsername" , "testPass" , 0.0 , null , null);
	...
	public void testAddServices() {
		...
		List getService =  session.createQuery("from ServicesOffered where workshop_workShopID = :user").setParameter("user", work.getWorkShopID()).list();
...
//			Object object = (Object) iterator.next();
	...
	public void testProvideFeedback() throws IncorrectFeedback {
		...
		Session session = sf.openSession();
		Transaction trans = session.beginTransaction();
		List feedbacks=  session.createQuery("from Feedback where workshop_workShopID = :user").setParameter("user", work.getWorkShopID()).list();
	...
	public static void tearDown()throws Exception {
		...
		Session session = sf.openSession();
		Transaction trans = session.beginTransaction();
		session.createQuery("delete from Feedback where workshop_workShopID = :user ").setParameter("user",work.getWorkShopID()).executeUpdate();
		session.createQuery("delete from Workshop where username = :user ").setParameter("user",work.getUsername()).executeUpdate();
