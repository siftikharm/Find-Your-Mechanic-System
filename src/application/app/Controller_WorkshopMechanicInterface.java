...
public class Controller_WorkshopMechanicInterface {
	...
	public void initialize() {
		...
		wCharges.setCellValueFactory(
				cellData -> new SimpleStringProperty(Integer.toString(cellData.getValue().getCharges())));
	...
	void addService(ActionEvent event) {
		ObjSingleton singleton = ObjSingleton.getInstance();
		Workshop cus = (Workshop) singleton.getS();
		ServicesOffered ser = new ServicesOffered(serviceName.getText(), serviceDesc.getText(),
				Integer.parseInt(serviceCharges.getText()));
